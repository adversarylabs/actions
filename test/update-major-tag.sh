...
case "$*" in
  *'/releases/latest --jq .tag_name') printf '%s\n' "$LATEST_TAG" ;;
...
log="$tmp/gh.log"
PATH="$tmp/bin:$PATH" GH_LOG="$log" LATEST_TAG=v1.4.7 RELEASE_SHA="$sha" \
...
: >"$log"
PATH="$tmp/bin:$PATH" GH_LOG="$log" LATEST_TAG=v2.0.0 RELEASE_SHA="$sha" REF_EXISTS=false \
...
if PATH="$tmp/bin:$PATH" GH_LOG="$log" LATEST_TAG=v1.4.7 RELEASE_SHA="$sha" \
