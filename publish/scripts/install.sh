...
curl --fail --silent --show-error --location "${base}/${archive}" --output "${download_dir}/${archive}"
curl --fail --silent --show-error --location "${base}/checksums.txt" --output "${download_dir}/checksums.txt"

expected="$(awk -v artifact="$archive" '$2 == artifact { print $1 }' "${download_dir}/checksums.txt")"
