...
When `cli-version` is omitted, the action installs its pinned default, `2026.9.11`. Set `cli-version` explicitly to test a different release. Pin the action itself to an exact release tag or full commit SHA for reproducible CI.
...
| Input | Required | Default | Description |
| --- | --- | --- | --- |
| `cli-version` | no | `2026.9.11` | Exact Adversary CLI release tag. |
| `path` | no | `.` | Adversary project directory. |
...
When `cli-version` is omitted, the action installs its pinned default, `2026.9.11`. Set `cli-version` explicitly to test a different release, and pin the action ref for reproducible CI. `path` defaults to `.`.
