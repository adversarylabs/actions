...
When `cli-version` is omitted, the action installs its pinned default, `2026.9.20-beta.1`. Set `cli-version` explicitly to test a different release. Pin the action itself to an exact release tag or full commit SHA for reproducible CI.
...
| Input | Required | Default | Description |
| --- | --- | --- | --- |
| `cli-version` | no | `2026.9.20-beta.1` | Exact Doomer CLI release tag. |
| `path` | no | `.` | Adversary project directory. |
...
When `cli-version` is omitted, the action installs its pinned default, `2026.9.20-beta.1`. Set `cli-version` explicitly to test a different release, and pin the action ref for reproducible CI. `path` defaults to `.`.
...
Push a new immutable `vMAJOR.MINOR.PATCH` tag from reviewed `main`. The release
workflow tests that tagged commit, installs its default CLI, updates and verifies
the moving major tag (`v1` for a `v1.x.x` release), then publishes the GitHub
